...
export function eventKey(event: SqliteEventWithNeighbours, idx: number): string {
	return event.eventId !== undefined ? String(event.eventId) : `idx:${idx}`;
}
...
export function scoreToOpacity(score: number, scoreMin: number, scoreMax: number, minOp = 0.5, maxOp = 1.0): number {
...
// Temporal profile
export function buildTemporalProfile(events: SqliteEventWithNeighbours[]): TemporalProfile {
...
export function toSeries(profile: TemporalProfile, token: string): TemporalPoint[] {
	const byYear = profile.get(token);
	if (!byYear) return [];
	return [...byYear.entries()].map(([year, set]) => ({ year, value: set.size })).sort((a, b) => a.year - b.year);
}
