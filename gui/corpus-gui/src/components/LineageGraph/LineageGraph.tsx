...
import type { LineageData, LineageNode } from "~/types/lineage";
...
const YEAR_RANGE_FROM_DATA = true;
...
export default function LineageGraph(props: LineageGraphProps) {
    ...
    function render(graph: LineageData) {
        ...
        // const years = d3.range(CORPUS_START_YEAR, CORPUS_END_YEAR + 1);

        const invalidYearNodes = graph.nodes.filter(
            (node) =>
                !Number.isInteger(node.year) ||
                node.year < CORPUS_START_YEAR ||
                node.year > CORPUS_END_YEAR,
        );

        if (invalidYearNodes.length > 0) {
            throw new Error(
                `Lineage data contains invalid year value(s): ` +
                invalidYearNodes.map((node) => `${ node.id } (${ String(node.year) })`).join(", "),
            );
        }
        ...
        const principalPoints = graph.nodes.map((node) => ({
            id: node.id,
            x: node.global?.x ?? node.local?.x ?? 0,
            y: node.global?.y ?? node.local?.y ?? 0,
        }));

        const invalidPrincipalPoints = principalPoints.filter(
            (point) => !Number.isFinite(point.x) || !Number.isFinite(point.y),
        );

        if (invalidPrincipalPoints.length > 0) {
            throw new Error(
                `Invalid global coordinates: ` +
                invalidPrincipalPoints
        ...
        graph.nodes.forEach((node, i) => {
            const px = x(node.year);
            const py = y(principalCoords[i]);

            if (px === undefined) {
                throw new Error(`No x-coordinate for lineage node ${ node.id } at year ${ node.year }`);
            }

            if (!Number.isFinite(py)) {
                throw new Error(`Invalid y-coordinate for lineage node ${ node.id }`);
            }

            positions.set(node.id, [px, py]);
        });
            ...
            circles
                ...
                .on("click", (event, d) => {
                    ...
                    if (selectedNode()?.id === d.id) {
                        target.classList.remove(styles.nodeSelected);
                        setSelectedNode(null);
                    } else {
                        target.classList.add(styles.nodeSelected);
                        setSelectedNode(d);
                    }
                });
