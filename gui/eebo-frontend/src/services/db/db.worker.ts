...
function execRows(sql: string, bind?: (string | number | null)[], traceId = "untracked"): unknown[][] {
...
async function init(url: string, traceId: string): Promise<void> {
	...
	// Sanity-check schema
	{
		...
		try {
			const rows = execRows(`SELECT name FROM sqlite_master WHERE type='table' ORDER BY name`, undefined, traceId);
			const tables = rows.map((r) => r[0] as string);
			if (!tables.includes("events")) {
				span.fail(new Error(`'events' table missing — found: [${tables.join(", ")}]`), { tables });
