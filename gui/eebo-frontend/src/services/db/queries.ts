...
export function buildEventQuery(): EventQuery {
	return {
		...
		selectedEventIds: controls.selectedEventIds ? Array.from(controls.selectedEventIds) : null,
	};
}
...
export async function fetchEvents() {
	...
	const conceptPlaceholders = concepts.length ? `IN (${concepts.map(() => "?").join(",")})` : "IS NOT NULL";
...
export async function queryYearBounds(concept: string): Promise<[number, number] | null> {
...
export async function queryEventById(id: string): Promise<SqliteEvent | null> {
	// An event and a neighbour are represented differently in the database,
	// but the caller needs one stable result shape.
	if (typeof id !== "string") {
	...
	let type: "event" | "neighbour" = "event";
	let row = eventRows[0];
	...
	return {
		...
		concept: row[1] != null ? String(row[1]) : null,
		vector_id: row[2] != null ? String(row[2]) : null,
		token: row[3] != null ? String(row[3]) : null,
		doc_id: row[4] != null ? String(row[4]) : null,
		...
		author: row[9] != null ? String(row[9]) : null,
		pub_place: row[10] != null ? String(row[10]) : null,
		score: row[11] != null ? Number(row[11]) : null,
	} as SqliteEvent;
}

export async function queryEventsByIds(ids: string[]): Promise<Map<string, SqliteEvent>> {
...
export async function getEventsByIds(ids: string[]): Promise<SqliteEvent[]> {
...
export async function queryNEvents(concept: string): Promise<number> {
	const rows = await execRows("SELECT n_events FROM concepts WHERE concept = ?", [concept]);
...
export async function queryYearCounts(concept: string): Promise<Map<number, number>> {
	const rows = await execRows(
