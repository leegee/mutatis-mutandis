...
export default function EvidenceForm(props: EvidenceFormProps) {
    ...
    const availableEntities = () => props.entities.filter(
        (entity) => !entityIds().includes(entity.id)
    );

    const availableRelations = () => props.relations.filter(
        (relation) => !relationIds().includes(relation.id)
    );

    const selectedEntities = () => props.entities.filter(
        (entity) => entityIds().includes(entity.id)
    );

    const selectedRelations = () => props.relations.filter(
        (relation) => relationIds().includes(relation.id)
    );


    function relationLabel(relation: Relation): string {
        const source = props.entities.find(
        ...
        <form onSubmit={submit}>
            ...
            <AutoComplete<Entity>
                ...
                onInput={(value) => {
                    ...
                    /*
                     * Typing a new value means the previous
                     * selection is no longer necessarily valid.
                     */
                    if (
                        sourceId() &&
                        value !==
