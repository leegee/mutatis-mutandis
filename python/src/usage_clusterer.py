...
Cluster diachronic neighbours of a concept (e.g., LAW) using HDBSCAN.

If TARGET is None, runs over all concepts in config.CONCEPT_SETS.
...
# Set to e.g. 'LAW' to restrict, or None to process all concepts
TARGET = None
...
def get_target_concepts() -> List[str]:
    if TARGET:
        return [TARGET.upper()]
    return list(config.CONCEPT_SETS.keys())
...
def main():
    ...
    logger.info(f"Wrote cluster results to {OUT_FILE}")
