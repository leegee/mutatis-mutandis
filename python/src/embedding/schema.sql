...
CREATE TABLE IF NOT EXISTS embedding.work (
    ...
    CONSTRAINT embedding_work_scale_ck
        CHECK (scale IN ('local', 'medium', 'broad')),
...
CREATE TABLE IF NOT EXISTS embedding.inventory (
    ...
    PRIMARY KEY (event_id, model_id, scale),

    CONSTRAINT embedding_inventory_scale_ck
        CHECK (scale IN ('local', 'medium', 'broad')),
...
CREATE INDEX IF NOT EXISTS embedding_inventory_model_event_idx
    ON embedding.inventory (model_id, scale, event_id);
