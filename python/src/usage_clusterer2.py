...
TARGET: Optional[str] = None  # set to a concept like 'LAW'
...
# Collect vectors
# ---------------------------
def collect_vectors(concept: str, *, include_freq: bool = True) -> Dict[str, List[Neighbour]]:
    """
    ...
    with eebo_db.get_connection() as conn:
        for slice_start, slice_end in config.SLICES:
            ...
            seed_vec = get_vector(conn, concept.lower(), slice_start, slice_end)
            if seed_vec is None:
                logger.warning(f"No vector for '{concept}' in slice {slice_key}")
                continue
            ...
            top_neighbors = [(t, s) for t, s in top_neighbors if t not in known_forms and t not in false_positives]

            if not top_neighbors:
                continue

            # Fetch frequencies from DB
            ...
            for token, sim in top_neighbors:
                ...
                freq_val = freq_map.get(token, 1) if include_freq else 1
                if freq_val is None:
                    freq_val = 1
                    logger.warning(f"Missing freq for token '{token}' in slice {slice_key}, defaulting to 1")
            ...
            if not all_slices[slice_key]:
                logger.warning(f"No valid neighbours collected for slice {slice_key}")
...
def assemble_clusters(neighbours: List[Neighbour], labels: np.ndarray, mass_modes: List[str]) -> Dict[int, Dict[str, Any]]:
    ...
    for cid, members in clusters.items():
        out[cid] = {
            ...
            "masses": {mode: MASS_FUNCTIONS[mode](members) for mode in mass_modes},
        }
    return out

def cluster_slices(all_slices: Dict[str, List[Neighbour]], *, mass_modes: List[str]) -> Dict[str, Any]:
    ...
    for slice_key, neighbours in all_slices.items():
        ...
        for cid, data in clusters.items():
            mass_str = ", ".join(f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}" for k, v in data["masses"].items())
            if cid == -1:
